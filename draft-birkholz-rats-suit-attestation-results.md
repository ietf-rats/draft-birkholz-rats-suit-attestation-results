...
In order to create Attestation Results, a Verifier consumes Evidence generated by a given Attester (amongst other Conceptual Messages, such as Endorsements and Attestation Policies).
...
The SUIT Workflow Model allows for two types of SUIT Procedures generating Reports on the Attester as defined in the SUIT Manifest specification {{-suit}}:
...
All Claims associated with an identifiable SUIT Manifest SHOULD always be bundled together in a Claims set. Claims collected in the context of SUIT MAY be bundled with Claims from other contexts -- but that composition requires careful consideration. Arbitrary composition of unrelated Claims can have a detrimental effect on appraisal procedures.
Additional Claims specified at a later point can be added to these Claims sets. The specification text describing additional Claims SHOULD elaborate on the relationship to existing Claims and highlight a preferred bundling.
...
For instance, a SUIT Vendor ID is either represented as an UUID or as a CBOR encoded Private Enterprise Number {{-cbor-oid}}.
...
The size of a firmware image on the Attester. This Claim MUST be bundled with a component-identifier and MAY be bundled with a image-digest.
...
The Version of a hardware or software component of the Attester. This Claim MUST be bundled with a component-identifier
